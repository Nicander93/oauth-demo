...
/**
 * OAuth 回调：换 token → 拉 UserInfo → 在本系统内解析/绑定本地用户。
 */
@Controller
public class CallbackController {
    ...
    public String callback(@RequestParam String code,
        ...
        if (result instanceof OAuthLoginService.OAuthLoginResult.LoggedIn loggedIn) {
            session.setAttribute(HomeController.SESSION_LOGIN, loggedIn.session());
            return "redirect:/";
        }
        if (result instanceof OAuthLoginService.OAuthLoginResult.NoLocalUser noLocal) {
            return "redirect:/?error=no_local_user&sub=" + noLocal.sub();
        }
